class Users::RegistrationsController < Devise::RegistrationsController
  ...
  def configure_account_update_params
    devise_parameter_sanitizer.permit(:account_update,
                                      keys: [:name, :location,
                                             :bio, :facebook_url,
                                             :itunes_url, :soundcloud_url,
                                             :spotify_url, :twitter_url,
                                             genres_list: []])
  end

  def update_resource(resource, params)
    params['genre'] = Genre.find(params[:genre].to_i) if params.has_key?(:genre)
    params['genres_list'].pop(1) if params.has_key?(:genres_list)
